...
export const MedicalProcedureForm = ({ route, service, id, show, refresh }) => {
  const form = useForm(
    {
  ...
  useEffect(() => {
    if (form.hasId) {
      form.handleFetch({
        action: (key, ac) => service.getById('medicalprocedure', id, ac.signal),
  ...
  return (
    <ModalForm
      isLarge
      title={form.displayName || 'Novo Procedimento'}
      show={show}
      ...
      onSubmit={() =>
        form.handleSubmit(data =>
          modalSubmit({ path: 'medicalprocedure', service, form, refresh, route, data }),
        )
      }
    >
      <div className="row">
