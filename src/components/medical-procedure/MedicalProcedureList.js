...
const columns = [
  ...
  { path: 'valueParticular', title: 'Valor Particular', format: 'money' },
  { path: 'valuePlan', title: 'Valor Plano', format: 'money' },
]

const fields = [...columns.map(c => c.path), 'disabledAt']

export const disableMedicalProcedure = ({
  modal,
  ...
  modal.confirm(
    ...
    confirmed =>
      confirmed &&
      service
        .remove('medicalprocedure', id)
        .then(callback)
        .catch(err => modal.alert(err.message)),
  )

export const MedicalProcedureList = ({ global, route, service, basename }) => {
  const { modal } = global
      ...
      <div className="kt-portlet kt-portlet--mobile">
        <div className="kt-portlet__head kt-portlet__head--lg">
          <div className="kt-portlet__head-label">
            <span className="kt-portlet__head-icon">
              <i className="kt-font-brand fas fa-hand-holding-medical" />
            </span>
            <h3 className="kt-portlet__head-title">Procedimentos</h3>
          </div>
            ...
            <div className="kt-portlet__head-wrapper">
              <div className="dropdown dropdown-inline">
                <Link
                  ...
                  to={`${basename}/procedimentos-medicos/cadastro`}
                >
                  <i className="fas fa-plus" /> Novo Procedimento
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className="kt-portlet__body kt-portlet__body--fit">
          ...
          <List
            ...
            fetching={fetching}
            filters={filters}
            initialQuery={{ fields, sort: [['description', 'ASC']] }}
            getItems={(query, signal) => service.getList('medicalprocedure', query, signal)}
            actions={[
              {
                icon: 'fas fa-edit',
                title: 'Editar',
                action: `${basename}/procedimentos-medicos/:id`,
              },
              {
                icon: 'fas fa-lock',
                title: 'Desabilitar',
