...
import { AttendanceRoomForm } from './AttendanceRoomForm'
...
export const disableRoom = ({ modal, data: { id, description, disabledAt }, callback, service }) =>
  modal.confirm(
    `Deseja ${!disabledAt ? 'desativar' : 'ativar'} ${description}?`,
    confirmed =>
      confirmed &&
      service
        .remove('attendanceroom', id)
        .then(callback)
        .catch(err => modal.alert(err.message)),
  )

export const AttendanceRoomList = ({ global, route, service, basename }) => {
  const { modal } = global
      ...
      <div className="kt-portlet kt-portlet--mobile">
        <div className="kt-portlet__head kt-portlet__head--lg">
          <div className="kt-portlet__head-label">
            <span className="kt-portlet__head-icon">
              <i className="kt-font-brand fas fa-door-open" />
            </span>
            <h3 className="kt-portlet__head-title">Salas de Atendimento</h3>
          </div>
            ...
            <div className="kt-portlet__head-wrapper">
              <div className="dropdown dropdown-inline">
                <Link
                  ...
                  to={`${basename}/salas-atendimento/cadastro`}
                >
                  <i className="fas fa-plus" /> Nova Sala de Atendimento
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className="kt-portlet__body kt-portlet__body--fit">
          ...
          <List
            ...
            filters={filters}
            initialQuery={{ fields }}
            getItems={(query, signal) => service.getList('attendanceroom', query, signal)}
            actions={[
              {
                icon: 'fas fa-edit',
                title: 'Editar',
                action: `${basename}/salas-atendimento/:id`,
              },
              {
                icon: 'fas fa-lock',
                title: 'Desabilitar',
